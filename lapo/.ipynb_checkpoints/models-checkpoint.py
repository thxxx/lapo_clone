...
class WorldModel(nn.Module):
    ...
    def __init__(self, action_dim, in_depth, out_depth, base_size=16):
        ...
        # downscaling
        print("월드 모델 : ", in_depth, action_dim, b)
        down_sizes = (in_depth + action_dim, b, 2 * b, 4 * b, 8 * b, 16 * b, 32 * b) # action_dim = 2 * 4 * 16 = 128
        ...
        for i, (in_size, out_size) in enumerate(partition(2, 1, down_sizes)):
            if i < len(down_sizes) - 2:
                ...
                self.down.append(
                    # nn.Conv2d(in_size, out_size, 2, 1)
                    nn.Identity()
                )
    ...
    def forward(self, state_seq, action):
        """
        state_seq.shape = (B, L, C, H, W) # 여기서 L=1, C=3
        action.shape = (B, action_dim) # B, L은 잘못됨!!
        """
        ...
        for i, layer in enumerate(self.up):
            x = layer(torch.cat([x, xs[-i - 1]], dim=1)) # dim=1이면 channel dimension에서
        
        out = self.final_conv(torch.cat([x, state], dim=1)) # 입력을 concat? 원래 하나?
        return F.tanh(out) / 2 # -> 값 범위 : (-0.5, 0.5)
...
if __name__ == "__main__":
    ...
    wm = WorldModel(action_dim=cfg.model.la_dim, in_depth=3, out_depth=3, base_size=24)
